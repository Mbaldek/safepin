  ...
  const search = useCallback((query: string, proximity?: [number, number]) => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(query)}.json?access_token=${MAPBOX_TOKEN}&country=fr&language=fr&types=place,address,poi${prox}&limit=5`
