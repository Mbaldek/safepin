// src/components/PinStoriesRow.tsx
...
export default function PinStoriesRow({ pinId }: { pinId: string }) {
  const { userId, userProfile } = useStore();
  const [stories, setStories] = useState<PinStory[]>([]);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => setStories((data ?? []) as PinStory[]));
  }, [pinId]);

  // Realtime new stories
  useEffect(() => {
    const ch = supabase
      .channel(`pin-stories-${pinId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'pin_stories', filter: `pin_id=eq.${pinId}` },
        (payload) => setStories((prev) => [payload.new as PinStory, ...prev]))
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [pinId]);

  // Group by user — latest story per user
  const storyByUser = new Map<string, PinStory>();
    ...
    <>
      ...
      <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
        ...
        <div className="flex flex-col items-center gap-1 shrink-0">
          ...
          <span className="text-[0.6rem] font-bold" style={{ color: 'var(--text-muted)' }}>Add</span>
        </div>
          ...
          return (
            <div key={s.user_id} className="flex flex-col items-center gap-1 shrink-0">
              <button
                ...
                style={{ padding: '2px', background: 'linear-gradient(135deg, #f43f5e, #f59e0b)' }}
              >
                <div
              ...
              <span className="text-[0.6rem] font-bold max-w-[56px] truncate" style={{ color: 'var(--text-muted)' }}>
